...
def render_page(node: NodeDoc, language: str) -> str:
    ...
    serialized_label = "**Nome serializado:**" if language == "pt" else "**Serialized name:**"
    source_label = "**Classe fonte:**" if language == "pt" else "**Source class:**"
    ...
    parts = [
        ...
        "",
        f"{serialized_label} `{node.serialized_name}`",
        "",
        f"{source_label} `{node.source_path.as_posix()}`",
        "",
